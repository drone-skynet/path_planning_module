...
def visualize_graph():
    for station in stations :
        ax.scatter(station.longitude, station.latitude, s=100, color='lightblue', zorder=1)  # 노드 크기와 색상 설정
    ...
    for edge in edges:
        ...
        ax.plot(x_values, y_values, color='gray', alpha=0.5, linewidth=1, zorder=1)  # 간선 그리기
...
if __name__ == '__main__':
    ...
    path = search_route(stations[2], stations[40])
    add_drone(path)
    print(path)

    path = search_route(stations[5], stations[86])
    add_drone(path)
    print(path)
    ...
    while(dest_sum > 0) :
        ...
        for drone in drones:
            ...
            # 드론의 현재 위치 표시
            ax.scatter(drone.longitude, drone.latitude, s=15, color='blue', alpha=0.5, zorder=2)
            if(drone.is_armed) :
                print(drone.id, drone.latitude, drone.longitude, drone.destinations[0].name)
