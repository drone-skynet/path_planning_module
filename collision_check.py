...
def check_collision_in_all_edges(edges) :
  for edge in edges :
    check_collision_in_edge(edge)


def check_all_collision(edges, intersections) :
  for drone in Drone.drones :
    if(drone.is_armed) :
      drone.go_flag = 1
  check_collision_in_all_edges(edges)
  check_collision_of_all_intersections(intersections)
  for drone in Drone.drones :
    if(drone.is_armed and drone.go_flag==1 and not drone.is_moving) :
      drone.move()
      print(drone.id, "드론 재가동")
    elif(drone.is_armed and drone.go_flag==0 and drone.is_moving) :
      drone.stop()
  return